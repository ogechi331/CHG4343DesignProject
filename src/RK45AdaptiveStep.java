...
public class RK45AdaptiveStep {
     ...
     * @author Dylan
     */
    public static double[] RK45 (double t, double y[], double timeStep, DifferentialEquation[] equation, double tolerance) {
        ...
        do {
            for (int i = 0; i<n; i++) {
                ...
                valueForK2[i] = y[i] + (K1 / 4);
            }
            for (int i = 0; i<n; i++) {
                K2[i] = wholeTimeStep * equation.apply(x, valueForK2[], timeStep, i);
                valueForK3[i] = y[i] + (3 * K1[i] / 32) + (9 * K2[i] / 32);
            }
            for (int i = 0; i<n; i++) {
                K3[i] = wholeTimeStep * equation.apply(x, valueForK3[], timeStep, i);
                valueForK4[i] = y[i] + (1932 * K1[i] / 2197) - (7200 * K2[i] / 2197) + (7296 * K3[i] / 2197);
            }
            for (int i = 0; i<n; i++) {
                K4[i] = wholeTimeStep * equation.apply(x, valueForK4[], timeStep, i);
                valueForK5[i] = y[i] + (439 * K1[i] / 216) - (8 * K2[i]) + (3680 * K3[i] / 513) - (845 * K4[i] / 4104);
            }
            for (int i = 0; i<n; i++) {
                K5[i] = wholeTimeStep * equation.apply(x, valueForK5[], timeStep, i);
                valueForK6[i] = y[i] - (8 * K1[i] / 27) + (2 * K2[i]) - (3544 * K3[i] / 2565) + 1859 * K4[i] / 4104 - (11 * K5[i] / 40);
            }
            for (int i = 0; i<n; i++) {
                K6[i] = wholeTimeStep * equation.apply(x, valueForK6[], timeStep, i);
            }
            for (int i = 0; i<n; i++) {
